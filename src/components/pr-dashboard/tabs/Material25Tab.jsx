...
const Material25Tab = ({ pr, onSave }) => {
  const { data, isPending, isFetching, isError, refetch } = useTabData('material25', pr);
  ...
  if (isError) return (
    <div className="text-center py-16">
      <p className="text-[#EF4444] font-bold text-sm">Failed to load Material 25</p>
  ...
  if (rows.length === 0) return <p className="text-[rgba(255,255,255,0.3)] text-sm py-8 text-center">No Material 25 data.</p>;
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-wider text-[rgba(255,255,255,0.35)] px-2 pb-2 border-b border-[rgba(255,255,255,0.06)] sticky top-0 bg-[#06090F] z-10">
        <IconPackage size={14} /><span>Material Items (25) — {rows.length} entries</span>
      </div>
      {rows.map((row, idx) => {
        const isExpanded = expandedRows[idx];
        const hasChanges = Object.keys(row).some((k) => k.startsWith('change') && row[k] !== null && row[k] !== '');
        return (
          <div key={row.id || idx}>
            <div className={`flex items-center gap-2 px-2 py-2.5 rounded-lg cursor-pointer transition-colors hover:bg-[rgba(255,255,255,0.02)] ${isExpanded ? 'bg-[rgba(245,158,11,0.04)] border border-[rgba(245,158,11,0.1)]' : 'border border-transparent'}`} onClick={() => toggleRow(idx)}>
              ...
              <span className="w-16 shrink-0 text-right text-[12px] font-semibold">{row.Qty || row.qty || row['Req Qty'] || row.req_qty || '0'}</span>
              ...
              <span className="w-20 shrink-0 text-[11px] text-[#F59E0B] truncate hidden sm:block">{row['Next Doc'] || row.next_doc || '—'}</span>
              {hasChanges && <span className="text-[9px] text-[#F59E0B] bg-[rgba(245,158,11,0.1)] px-1.5 py-0.5 rounded font-bold">Changes</span>}
            </div>
            {isExpanded && (
              <div className="ml-10 mb-3 p-4 bg-[rgba(255,255,255,0.01)] border border-[rgba(255,255,255,0.06)] rounded-xl space-y-4">
                {hasChanges && <ChangeTimeline row={row} changeCount={5} />}
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 pt-2 border-t border-[rgba(255,255,255,0.06)]">
                  ...
                  <div><span className="text-[9px] font-bold uppercase text-[rgba(255,255,255,0.25)]">Next Doc</span><InlineCell value={row['Next Doc'] || ''} row={row} column="Next Doc" onSave={onSave} /></div>
                </div>
              </div>
            )}
          </div>
